...
describe('WorkoutLogger', () => {
    let mockGetWorkouts;
    let mockAddWorkout;
    let mockGetExercises;
    let mockGetWorkoutTemplates;
    beforeEach(() => {
        localStorage.clear();
        jest.restoreAllMocks();
        mockGetWorkouts = jest.spyOn(workoutsApi, 'getWorkouts').mockResolvedValue([]);
    ...
    function getExerciseCardElement(exerciseId, exerciseName) {
    ...
    it('adds a workout entry and displays it in history (mocks API)', async () => {
        ...
        mockAddWorkout.mockResolvedValue(newEntry);
        renderWithProvider(_jsx(WorkoutLogger, {}));
        await waitFor(() => expect(mockGetWorkouts).toHaveBeenCalled());
    ...
    it('configures exercise parameters in structured mode', async () => {
        ...
        const setsInput = within(card).getByLabelText(/sets/i);
        const repsInput = within(card).getByLabelText(/reps/i);
        const weightInput = within(card).getByLabelText(/weight \(lbs\)/i);
    ...
    it('saves structured workout with exercises', async () => {
        ...
        const nameInput = screen.getByPlaceholderText(/e.g., Upper Body Strength/i);
